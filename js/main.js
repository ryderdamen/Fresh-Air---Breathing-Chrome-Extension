...
function initializePage() {
	...
	[	'fa_inhaleLength',
		...
		if (stored.fa_startAutomatically) {
			doBreathing(bar, stored.fa_inhaleLength, stored.fa_holdLength);
		}
		else {
			// Or, Set up clicking so that it's ready to go for the user
			$( "#circle" ).click(function() { doBreathing(bar, stored.fa_inhaleLength, stored.fa_holdLength); });
		}
...
function doBreathing(bar, breathLength, holdLength) {
	...
	$( "#circle" ).click(function() { doBreathing(bar, breathLength, holdLength); });
